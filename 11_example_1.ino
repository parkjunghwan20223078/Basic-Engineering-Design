...
#define PULSE_DURATION 100 // ultra-sound Pulse Duration (unit: usec)
...
void loop() {
  ...
  if (dist_raw < _DIST_MIN) {
    dist_raw = _DIST_MIN;           // cut lower than minimum
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (dist_raw > _DIST_MAX) {
    dist_raw = _DIST_MAX;           // Cut higher than maximum
    digitalWrite(PIN_LED, 1);       // LED OFF
  ...
  dist_ema = (1-_EMA_ALPHA)*dist_ema + dist_raw*_EMA_ALPHA;
  ...
  if (dist_raw <= _DIST_MIN){
      ...
      dist_raw = dist_ema;
      myservo.writeMicroseconds(int (185*dist_raw/18 - 1300));
  }
  else{
